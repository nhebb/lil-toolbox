...
export class BackingPropsService {
  ...
  public createBackingProps(input: string): string {
    const viewModelItems = this.getViewModelItems(input);
    let output = this.generateCode(viewModelItems);
    if (output.length === 0) {
...
private generateCode(viewModelItems: ViewModelItem[]): string
{
  const output: string[] = [];
  for (const item of viewModelItems) {
    if (item.isButton) {
        ...
        this.generateCollectionCode(item, output);
    }
    else {
        this.generateStandardCode(item, output);
    }
  }
  return output.join('\r\n');
}
...
private generateCollectionCode(item: ViewModelItem, output: string[]): void {
...
private generateStandardCode(item: ViewModelItem, output: string[]): void {
    const indent = '    ';
